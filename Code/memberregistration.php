...
// 
	...
	if(isset($_FILES['mempic'])){
    $errors= array();
    $file_name = $_FILES['mempic']['name'];
    $file_size =$_FILES['mempic']['size'];
    $file_tmp =$_FILES['mempic']['tmp_name'];
    $file_type=$_FILES['mempic']['type'];   
    $exploded = explode('.',$_FILES['mempic']['name']);
    ...
    if(empty($errors)==true){
        move_uploaded_file($file_tmp,"mempic/".$file_name);
...
if($a=='1')
	...
	else
	{
								...
								$qry="Insert into member_master(areaid,memfname,memlname,uname,pwd,doreg,dateoflv,vcnt,contact,email,issmoking,isdiabic,bloodpressure,gender,mempic,isactive,address) values('$areaname','$fn','$ln','$un','$pass','$dtofreg','$dtoflast','$vcnt','$conta','$email','$smoking','$diabetic','$Bloodpressure','$gender','$file_name','$active','$address')";
								// echo $qry;
								$cnn = mysqli_connect("localhost","root","","careathome");
...
<div class="page-header">
						...
						<form class="form-horizontal" role="form" method="post" enctype="multipart/form-data">
									...
									<div class="form-group">
										...
										<div class="col-sm-9">
											<input type="text" placeholder="Contact" class="col-xs-10 col-sm-5" name="txtcontact" minlength="10" maxlength="10" required="" pattern="[6|7|8|9][0-9]{9}">
										</div>
									</div>	
